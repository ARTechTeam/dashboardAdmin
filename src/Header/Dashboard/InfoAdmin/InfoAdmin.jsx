  ...
  return (
    <div className="flex flex-col gap-3 justify-center items-center py-3 border-b border-light-border dark:border-dark-border">
      <button className="border-8 rounded-full dark:border-Secondary/30 border-Secondary/15 w-fit h-fit relative">
        <img
          src={`${adminInformation.photo == 'default' ? '/IMAGE/userDefaultImage.png' : adminInformation.photo} `}
          className=""
          alt=""
        />
      ...
      <h2 className="text-Primary dark:text-Secondary-light font-primary-bold">{adminInformation.name}</h2>
      <figcaption className="text-gray-400">{adminInformation.subTitle}</figcaption>

      <div className="flex gap-0 items-center justify-center sp">
        {adminInformation.account.map((item, i) => (
          <div
            className={`flex flex-col justify-center items-center gap-1 ${
              i !== 0 ? 'border-r border-light-border dark:border-dark-border' : ''
            } px-5`}
            key={item[2]}
          >
