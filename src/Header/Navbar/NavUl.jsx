...
export default function NavUl({ changeMode }) {
  ...
  const my_navbar = [
    ...
    {
      title: '',
      icon: isNight ? <IoSunnyOutline className="text-xl" /> : <IoMoonOutline className="text-xl" />,
      id: 4,
      onClick: () => {
        changeMode();
        changeNight();
      ...
      {my_navbar.map((item) => (
        <li key={item.id}>
          <Btn onClickHandler={item.onClick} title={item.title}>
            ...
            {item.title && <div className="line-clamp-1 truncate dark:font-primary-bold">{item.title}</div>}
          </Btn>
        </li>
      ))}
    </ul>
